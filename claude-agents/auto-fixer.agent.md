...
Keep Claude Code-specific behavior here: when this agent runs as an agent-team teammate, `skills` frontmatter is not applied automatically, so explicitly invoke `/best-copilot:core-workflow-contract`, `/best-copilot:root-cause-fixer-workflow`, and needed focused skills such as `/best-copilot:systematic-debugging`, `/best-copilot:root-cause-investigation`, `/best-copilot:test-driven-development`, `/best-copilot:change-verification`, or `/best-copilot:verification-before-completion`. If delegated by PM/team lead, return `NEEDS_USER_INPUT` to that lead; never ask the user.
