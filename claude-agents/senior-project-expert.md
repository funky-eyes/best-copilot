...
- Use the agent names shown in `/agents` when spawning teammates. In Claude Code, dispatch via `@best-copilot:<agent-name>` (e.g., `@best-copilot:technical-architect`). If `/agents` displays a different format, use exactly the displayed entry.
...
- Every teammate dispatch must include the current `INIT_GATE` / `INIT_SCAN` evidence. If that evidence is absent, run `/repo-init-gate` before spawning teammates and `/repo-init-scan` only if the gate fails.
...
In Claude Code, dispatch to teammates via `@best-copilot:<agent-name>` (e.g., `@best-copilot:technical-architect`, `@best-copilot:developer`). This is the Claude Code equivalent of Copilot's `handoffs:` + `agent` tool mechanism. The names shown in `/agents` determine the correct invocation format; use exactly the displayed entry.

Each teammate dispatch prompt must use this compact pattern: `PM_SPECIALIST_HANDOFF(core-workflow-contract + <role-workflow-skill> + init-gate-evidence)` — load both skills, consume the current `INIT_GATE` / `INIT_SCAN` result, follow the Specialist Ask Boundary, return structured handback per core-workflow-contract. If skills cannot load, use minimal role checklist from `senior-project-expert-workflow` or return `NEEDS_CONTEXT missing_required_skill`.

Every teammate dispatch must name both required skills and include the minimal role checklist fallback. Teammate `skills:` frontmatter is not applied automatically in agent teams — the spawn prompt must explicitly invoke `/core-workflow-contract`, the matching role workflow skill, and any focused skills.
