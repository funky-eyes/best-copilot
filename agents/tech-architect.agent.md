...
- For SDD design, produce a design-time assignment matrix: each slice names owner lane, reviewer lanes, write set, dependencies, parallel group/readiness, acceptance checks, verification command, ready artifacts, and stop conditions. Split tasks that mix unrelated write sets or are too broad for a fresh-context specialist to understand in 2-5 minutes.
...
- Invoke `verification-before-completion` before any final user-facing completion claim. Use `structured-review`, `spec-execution-fastpath`, and `change-verification` when their trigger conditions apply; `technical-architect-workflow` enforces TDD directly.
