...
- Invoke `verification-before-completion` before any final user-facing completion claim. Use `target-spec-bootstrap`, `target-memory-bootstrap`, and `context-packet-fastpath` when their trigger conditions apply; `specification-writer-workflow` owns planning directly. Use `repo-init-gate` first and invoke `repo-init-scan` only after that gate fails, or when target-local scaffolds still need repair.
