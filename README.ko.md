...
기본 multi-agent 흐름은 PM 메인 세션 + Agent tool subagents를 사용하므로 실험적 Agent Teams가 필요하지 않습니다. Claude Code의 실험적인 공유 팀 runtime을 명시적으로 사용하려는 경우에만 Agent Teams를 활성화하세요:
...
Claude 어댑터는 `model: inherit`을 사용하므로 실제 모델은 현재 Claude Code 세션의 `/model`, `--model`, 또는 사용자 설정이 결정합니다. Copilot의 크로스 프로바이더 모델 라우팅을 복제하지 않습니다.
...
| 모델 지정 | `GPT-5.4 (copilot)` 등 구체적 이름 | `model: inherit` (사용자 설정) |
...
| 크로스 모델 라우팅 | 지원 (GPT / Gemini / Claude 혼합) | Claude 모델만 |
