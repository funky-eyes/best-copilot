...
Codex는 `.codex-plugin/plugin.json`, `.agents/plugins/marketplace.json`, `.agents/skills -> ../skills`를 사용합니다. Copilot CLI는 `plugin.json`을 통해 루트 `agents/`와 `skills/`를 사용합니다. Claude Code는 `claude-plugin/` 패키지를 사용합니다: `claude-plugin/.claude-plugin/plugin.json`, `claude-plugin/skills -> ../skills`, `claude-plugin/agents -> ../claude-agents`. 저장소 수준 규칙은 `.github/instructions/**`에 보관됩니다.
...
설치하거나 수정한 뒤에는 새로운 Codex thread/session을 시작하세요. 이 workflow를 사용할 때는 `@best-copilot` 또는 bundled `$skill`을 명시적으로 호출합니다.
...
요구사항 오케스트레이션은 **Senior Project Expert** PM 코디네이터에서 시작합니다. 이 역할은 의도, 범위, 계획, 디스패치, 리뷰 fan-in, 종료를 담당합니다.
...
8. PM이 모든 서브에이전트 결과를 수집하여 fan-in 중재
9. `/best-copilot:verification-before-completion` 호출 후 사용자에게 전달
...
Claude agent frontmatter는 일반적으로 `core-workflow-contract`와 해당 역할 workflow만 미리 로드합니다. Senior Project Expert는 init preflight가 필수 부트 게이트이므로 `repo-init-gate`와 `repo-init-scan`도 미리 로드합니다. `structured-review`, `test-driven-development`, `web-experience-audit` 같은 다른 focused skills는 agent 본문에서 필요할 때만 호출하도록 두어 시작 컨텍스트를 줄입니다.
...
```
INIT_GATE → [필요시 INIT_SCAN] → CLASSIFY → FREEZE_PACKET → LANE_SELECTION
  → [전체/모호/고위험시 ARCHITECT_SDD] → REVIEW_OR_DISPATCH
...
대상 저장소에서 실질적인 작업을 하기 전, 시스템은 먼저 `repo-init-gate`를 실행합니다 — 대상 저장소 루트의 `best-copilot.md`만 읽어서 frontmatter의 `version`이 현재 계약 버전 `"0.7.0"`과 일치하는지 확인합니다.
...
종료 전, 시스템은 `verification-before-completion` 최종 검사를 실행합니다:
...
│   └── workflow-rules.md          ← 메모리/스펙 조정 규칙
│
...
- EvolutionEvent 기록에는 signal, target, mutation, validation, rollback, status 모든 필드 필요
...
```
작업 실행 → 신호 생성 (실패/수정/마찰)
    │
    ▼
evolution-loop 스킬 개입
    │
    ▼
최소 개선 목표 선택
(agent / skill / instruction / memory / spec template)
    │
    ▼
경계 있는 변이 제안 (Evolution Proposal)
    │
    ▼
검증 (정적 검사 / 평가 프롬프트 / 리뷰 / 명령 증거)
    │
    ├── 수용 → canonical root에 기록
...
2. **최소 목표**:每次 최소의 재사용 가능한 개선 목표만 변경, 대규모 재작성 금지
...
5. **반드시 검증 필요**:每次 변에는 정적 검사, 평가 프롬프트, 리뷰 또는 명령 증거 필요
