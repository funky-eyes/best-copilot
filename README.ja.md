...
- セッション agent は `--agent senior-project-expert` または Claude Code のプロジェクト/ユーザー `agent` 設定で選択します。`/agents` に表示される `best-copilot:` prefix なしの名前を使います。
...
その後、Claude Code では `--agent senior-project-expert`、またはプロジェクト/ユーザー `agent` 設定を `senior-project-expert` にして実作業を始めます。対象のリポジトリ内で有用な事実を正規化し、欠けているローカルのスキャフォールドを作成し、実装に入る前にこれらのファイルが存在することを検証します。
