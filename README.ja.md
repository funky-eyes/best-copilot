...
- **ひとつのシニア入口**：Senior Project Expert が意図、範囲、ディスパッチ、fan-in、集約、再利用可能なリフレクション/Memory/ポリシーシグナルを担います。
...
要求のオーケストレーションは **Senior Project Expert** PM コーディネーターから始めます。意図、範囲、計画、ディスパッチ、レビュー fan-in、リフレクション、Memory 同期、ポリシー差分ルーティング、クローズを担当します。
...
8. PM がすべてのサブエージェント結果を `OBSERVER_FAN_IN` として収集し、`EVALUATOR_ARBITRATION` を実行
9. `REFLECTOR_SIGNAL` を記録し、`MEMORY_STATE_SYNC` を実行し、採用済みで検証済みの学習にのみ `POLICY_DELTA_OR_NONE` を適用
10. `/best-copilot:verification-before-completion` を呼び出してからユーザーに提供
...
```
INIT_GATE → [必要に応じて INIT_SCAN] → CLASSIFY → PLANNER_FREEZE_PACKET
  → [full/曖昧/high-risk の場合 ARCHITECT_SDD] → LANE_SELECTION
...
`PLANNER_FREEZE_PACKET` と実行段階では、次の制約を保持します：
...
### ステージ 3: Planner コンテキスト固定（6ブロックディスパッチパケット）
...
### ステージ 6: Observer Fan-In と Evaluator 仲裁

PM はまずすべての専門家 handback を観測可能な証拠として記録し、その後優先度に従って結果を裁定します：
...
### ステージ 7: リフレクション、Memory 同期、ポリシー差分、検証、クローズアウト

クローズ前に、PM は自己進化の末尾ループを完了する必要があります：

- `REFLECTOR_SIGNAL`：検証済みの再利用可能な教訓を抽出する、または `evolution_signal: none` を記録
- `MEMORY_STATE_SYNC`：進捗、検証、クローズ、進化ステータスが変わったとき task/spec/memory 状態を同期
- `POLICY_DELTA_OR_NONE`：採用済み、検証済み、ロールバック可能な教訓に限り、ルーティング、prompt、テンプレート、ツール優先度、workflow ルールを更新

その後、システムは `verification-before-completion` 最終チェックを実行します：
...
│   ├── decisions.md               ← 意思決定記録
│   └── workflow-rules.md          ← メモリ/スペック/evolution 調整ルール
│
...
- EvolutionEvent 記録には signal、target、mutation、validation、rollback、seven_module_trace、ten_pass_review、status のすべてのフィールドが必要です
...
```
Planner → 改善目標、対象、検証、ロールバックを固定
Executor → 最小の承認済み変更を実行、または提案を返す
Observer → 具体的シグナルと再生可能な証拠を取得
Evaluator → 深刻度、再発性、信頼度、影響範囲を評価
Reflector → 根本原因、教訓、将来アクション、反パターンを抽出
Memory → canonical owner に accepted/rejected/deferred event を記録
Policy → 検証後にのみ境界のある workflow ルールを更新
```
...
- seven_module_trace: ← Planner/Executor/Observer/Evaluator/Reflector/Memory/Policy の証拠
- ten_pass_review: ← source priority、分離、スコープ、能力低下なし、証拠、memory hygiene、可逆性、runtime 互換、検証適合、将来再利用
