...
Claude アダプターは `claude-agents/*.md` で Claude モデル alias を使います：GPT-5.4 対応ロールは `opus`、Gemini 対応ロールは `haiku`、Claude Sonnet ロールは `sonnet` です。Claude Code は Claude モデルのみを実行し、この対応で Copilot 側のロール階層を Claude のモデル群に写します。
...
Claude Code は Claude モデルのみを実行します。`claude-agents/*.md` の Claude アダプターは役割分離を保ち、Copilot 階層を Claude alias に対応させます：GPT-5.4 -> `opus`、Gemini 3.1 Pro (Preview) -> `haiku`、Claude Sonnet 4.6 -> `sonnet`。
