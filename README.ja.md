...
- **39 のスキル**：ロール workflow、ブートストラップ、検索、計画、workspace isolation、TDD、設計レビュー、実行、Java/Python コーディングガイドライン、検証、branch closeout、フロントエンド監査、ワークフローの進化、Senior Project Expert 互換エントリポイントなどを提供します。
...
- 共有スキル：[skills/](skills/)。Claude Code では `/repo-init-gate` のような裸の slash command で呼び出し、コマンドピッカーの説明列に `(best-copilot)` が表示されます。
...
- **Claude Code**：`claude --plugin-dir /absolute/path/to/best-copilot/claude-plugin` で開始します。プラグインの既定 agent 設定は `best-copilot:senior-project-expert` ですが、CLI は通常 prefix なしの agent / skill 名を表示します。`/agents` でプラグイン agent を確認し、`/repo-init-gate` のような裸の slash command で skill を呼び出せます。

Claude Code が Senior Project Expert の要求を subagent ではなく `Skill(senior-project-expert)` として解決した場合、同名の互換 skill が同じ repo-init preflight を実行してから分析、計画、ディスパッチ、実装を許可します。完全な Senior Project Expert 経路では `/agents` または agent teams を優先してください。
...
その後、Claude Code では **Senior Project Expert** / `senior-project-expert` から実作業を始めます。対象のリポジトリ内で有用な事実を正規化し、欠けているローカルのスキャフォールドを作成し、実装に入る前にこれらのファイルが存在することを検証します。
