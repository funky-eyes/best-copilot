...
Claude アダプターは `claude-agents/*.md` で Claude モデル alias を使います：GPT-5.4 対応ロールは `opus`、Gemini 対応ロールは `haiku`、Claude Sonnet ロールは `sonnet` です。ネイティブ Claude Code では、これらの alias によって Copilot 側のロール階層を Claude のモデル群内に保ちます。`cc-switch`、`new-api`、または他の Anthropic-compatible proxy がこれらの alias を DeepSeek、Qwen、その他の非 Claude backend にルーティングする場合、hook gate と workflow smoke check の両方を通過するまでは degraded とみなしてください。plugin は初回用の `UserPromptSubmit`、`UserPromptExpansion`、`PreToolUse` hooks を提供し、init-gate context を注入し、展開前に非 init の best-copilot slash command をブロックし、target の `best-copilot.md` が current になるまで business-source tools を拒否します。target bootstrap はコミット可能な `.claude/hooks/best-copilot-init-gate.sh` fallback も生成できますが、bootstrap が開始した後にのみ有効です。PM はその後、source reading や implementation の前に `PROVIDER_COMPAT -> INIT_GATE -> CLASSIFY -> FREEZE_PACKET -> LANE_SELECTION` を出力し、必要な specialist lanes を明示する必要があります。
...
ネイティブ Claude Code は Claude モデル alias を使います。`claude-agents/*.md` の Claude アダプターは役割分離を保ち、Copilot 階層を Claude alias に対応させます：GPT-5.4 -> `opus`、Gemini 3.1 Pro (Preview) -> `haiku`、Claude Sonnet 4.6 -> `sonnet`。`cc-switch` や `new-api` などの proxy route は、これらの alias を非 Claude モデルにマップすることがあります。これは API 互換であり、workflow 互換を意味しません。Claude plugin には hook-level enforcement が含まれます。target sentinel が current になる前は、`PreToolUse` が business-source tools と codegraph/MCP tools を拒否し、sentinel read、init bootstrap write、bounded root discovery のみを許可します。この plugin hook が first-run guard です。target bootstrap は後で `.claude/settings.json` hook entries と `.claude/hooks/best-copilot-init-gate.sh` を committed fallback policy として追加できますが、target-local hooks は自分自身を bootstrap できません。DeepSeek、Qwen、unknown backend では、実作業の前に非破壊の workflow smoke check を実行してください。期待される動作は、PM が `PROVIDER_COMPAT -> INIT_GATE -> CLASSIFY -> FREEZE_PACKET -> LANE_SELECTION` を出力し、その後 request に必要な lanes を dispatch することです。hook feedback 後も coding を始めたり lanes をスキップする場合は、smoke check を通過する model/provider を使ってください。
