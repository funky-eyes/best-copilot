...
デフォルトのマルチエージェントフローは PM メインセッション + Agent tool subagents を使うため、実験的な Agent Teams は不要です。Claude Code の実験的な共有チーム runtime を明示的に使いたい場合だけ Agent Teams を有効にします：
...
Claude アダプターは `model: inherit` を使うため、実際のモデルは現在の Claude Code セッションの `/model`、`--model`、またはユーザー設定で決まります。Copilot のクロスプロバイダモデルルーティングは再現しません。
...
| モデル指定 | `GPT-5.4 (copilot)` 等の具体的名前 | `model: inherit`（ユーザー設定） |
...
| クロスモデルルーティング | サポート（GPT / Gemini / Claude 混在） | Claude モデルのみ |
...
`current-workstreams.md` の各ワークフローは対応するスペックを指す `linked_spec` を持ちます。`spec/INDEX.md` の各スペックは関連メモリを逆参照できます。EvolutionEvent 記録には signal、target、mutation、validation、rollback、status のすべてのフィールドが必要です。
