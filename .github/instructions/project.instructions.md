...
- Primary Codex install surfaces: root `.codex-plugin/plugin.json` for Codex plugin metadata, plus root `.agents/plugins/marketplace.json` for local/repo Codex marketplace discovery.
...
- Current compatibility targets: Codex plugin/skill loading, Copilot CLI plugin installation, and Claude Code plugin loading.
- Runtime adapter architecture: shared cross-role behavior lives in `skills/core-workflow-contract/SKILL.md`; role-specific workflow behavior lives in matching `skills/*-workflow/SKILL.md` files; Copilot-only model/tool/handoff metadata lives in `agents/*.agent.md`; Claude-only runtime agent adapter names, Claude model aliases, and agent-team limitations live in runtime-named `claude-agents/*.md` files; Codex-specific install metadata lives in `.codex-plugin/` and `.agents/`, while workflow behavior remains in shared skills.
...
For Codex distribution, users should add the marketplace with `codex plugin marketplace add funky-eyes/best-copilot`, install with `codex plugin add best-copilot@best-copilot`, then start a new Codex thread and invoke `@best-copilot` or a bundled `$skill` explicitly when they want this workflow. For local development from this checkout, `.agents/plugins/marketplace.json` exposes `plugins/best-copilot` as the installable plugin package and `.agents/skills` exposes the shared skills without copying them. Codex subagent workflows require explicit user or PM instruction to spawn parallel agents; Codex does not auto-spawn subagents solely because a plugin is installed.
