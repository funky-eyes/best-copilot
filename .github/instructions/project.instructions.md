...
- Code intelligence MCP is optional in Claude Code target environments. Prefer `mcp__gitnexus__*` when present, else `mcp__codegraph__*`, else built-in Read/Grep/Glob plus shell `rg`. Missing code intelligence must not block init, dispatch, review, or implementation, and absent tools must not be called.
