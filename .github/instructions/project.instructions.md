...
- In Claude Code, `Skill(...) Successfully loaded` is not initialization evidence. First-use init is complete only after the scan report records disk verification, `required_artifacts_verified: yes`, `sentinel_written: yes`, and `next_task_ready: yes`.
