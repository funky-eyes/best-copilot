...
- Purpose: installable Codex, Copilot CLI, and Claude Code agent-team workflow plugin plus reusable repository workflow customization templates.
...
- Primary Codex install surfaces: root `.codex-plugin/plugin.json` for Codex plugin metadata, root `.agents/plugins/marketplace.json` for local/repo Codex marketplace discovery, root `.agents/skills -> ../skills` for normal `$` skill discovery from this checkout, and root `.codex/agents/*.toml` for Codex custom agent discovery from this checkout or target repos that copy these adapters.
...
- Codex discovery source: `.codex-plugin/plugin.json` declares root `skills/` for direct package use; `.agents/plugins/marketplace.json` points at `plugins/best-copilot`, a real Codex plugin subpackage with `plugins/best-copilot/skills -> ../../skills`, because Codex CLI enumerates plugin sources under marketplace plugin paths rather than the marketplace root itself; `.agents/skills -> ../skills` exposes the same shared skills for direct repo-scoped Codex use from this checkout; `.codex/agents/*.toml` exposes the role adapters as Codex custom agents for direct checkout or target-repo use.
...
- Repository instruction source: `.github/instructions/**`; target repositories that need Codex compatibility should get a root `AGENTS.md` adapter, and target repositories that need Claude Code compatibility should also get a root `CLAUDE.md` adapter that imports those shared instruction files.
- Current compatibility targets: Codex plugin/skill loading, Codex custom agent adapter loading, Copilot CLI plugin installation, and Claude Code plugin loading.
- Runtime adapter architecture: shared cross-role behavior lives in `skills/core-workflow-contract/SKILL.md`; role-specific workflow behavior lives in matching `skills/*-workflow/SKILL.md` files; Copilot-only model/tool/handoff metadata lives in `agents/*.agent.md`; Claude-only runtime agent adapter names, Claude model aliases, and agent-team limitations live in runtime-named `claude-agents/*.md` files; Codex-specific install metadata lives in `.codex-plugin/` and `.agents/`, Codex custom agent adapters live in `.codex/agents/*.toml`, and workflow behavior remains in shared skills.
...
- Claude Code adapters normally preload only `core-workflow-contract` and their matching role workflow skill in frontmatter. Senior Project Expert additionally preloads only `repo-init-gate` so repeat requests can use the cheap `best-copilot.md` sentinel fast path; `repo-init-scan` stays on-demand and is loaded only when the gate fails. In shell-capable Claude Code, prefer the bundled `repo-init-gate/scripts/run-preflight.sh` helper so a failed gate immediately runs the scan bootstrap instead of relying on a second skill-load step. Other focused skills stay on-demand in the agent body to avoid unnecessary startup context.
- Code intelligence MCP is optional in Claude Code target environments. Prefer `mcp__gitnexus__*` when present, else `mcp__codegraph__*`, else built-in Read/Grep/Glob plus shell `rg`. For TypeScript/JavaScript, use exposed LSP tools or diagnostics from `typescript-lsp@claude-plugins-official` before grep fallback when that plugin and language server capability are actually available. Missing code intelligence must not block init, dispatch, review, or implementation, and absent tools must not be called.
...
- Subsequent conversations should invoke `repo-init-gate` and read only the target root `best-copilot.md` first. If that file's YAML frontmatter contains `version: "0.7.0"`, skip full repo-init artifact revalidation.
...
- Plugin installation contributes workflow packaging and, in runtimes that support them as plugin components, skills and agents; Codex currently treats custom agents as separate `.codex/agents` or `~/.codex/agents` TOML config rather than plugin-installed components, so this checkout ships `.codex/agents/*.toml` adapters for direct repo/target-repo exposure. It does not provide shared per-project memory storage.
...
| List Codex custom agent adapters | `find .codex/agents -maxdepth 1 -name "*.toml" | sort` |
| List Claude Code agent adapters | `find claude-agents -maxdepth 1 -name "*.md" | sort` |
| Check JSON manifest | `ruby -rjson -e 'JSON.parse(File.read("plugin.json")); JSON.parse(File.read(".codex-plugin/plugin.json")); JSON.parse(File.read("claude-plugin/.claude-plugin/plugin.json")); JSON.parse(File.read(".claude-plugin/marketplace.json")); JSON.parse(File.read("settings.json")); JSON.parse(File.read(".claude/settings.json")); puts "json ok"'` |
| Check marketplace catalog | `ruby -rjson -e 'JSON.parse(File.read("marketplace.json")); JSON.parse(File.read(".github/plugin/marketplace.json")); JSON.parse(File.read(".agents/plugins/marketplace.json")); puts "marketplace json ok"'` |
...
For Codex distribution, users should add the marketplace with `codex plugin marketplace add funky-eyes/best-copilot`, install with `codex plugin add best-copilot@best-copilot`, then start a new Codex thread and invoke `@best-copilot` explicitly when they want this workflow. In current Codex CLI behavior, installed plugin details may list bundled skills while the normal `$` skill selector only shows skills exposed through repo/user/system skill locations. For local development from this checkout, `.agents/plugins/marketplace.json` exposes `plugins/best-copilot` as the installable plugin package, `.agents/skills` exposes the shared skills without copying them, and `.codex/agents/*.toml` exposes the role adapters as Codex custom agents. Codex subagent workflows require explicit user or PM instruction to spawn parallel agents; Codex does not auto-spawn subagents solely because a plugin is installed, and plugin installation does not register `agents/*.agent.md` as Codex custom agents.
...
- In Claude Code, the concrete native ask mechanism is `AskUserQuestion`: use 1 question by default, 2-4 options with short descriptions, and the built-in custom/Other answer path. A prose next-step question such as "continue implementation or run E2E first?" is invalid when `AskUserQuestion` is available.
...
- For standard/full target-repository tasks, Senior Project Expert should not call codegraph/read/search across business source as a substitute for specialists. Before init completes, only init-scoped evidence is allowed; after init, named specialist lanes own business-code inspection and hand back structured evidence.
