...
- Codex discovery source: `.codex-plugin/plugin.json` declares root `skills/`; `.agents/skills -> ../skills` exposes the same shared skills for direct repo-scoped Codex use from this checkout.
...
For Codex distribution, users should add the marketplace with `codex plugin marketplace add funky-eyes/best-copilot`, install from the plugin directory, then start a new Codex thread and invoke `@best-copilot` or a bundled `$skill` explicitly when they want this workflow. For local development from this checkout, `.agents/plugins/marketplace.json` exposes the root plugin and `.agents/skills` exposes the shared skills without copying them. Codex subagent workflows require explicit user or PM instruction to spawn parallel agents; Codex does not auto-spawn subagents solely because a plugin is installed.
