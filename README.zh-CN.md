...
- 共享技能：[skills/](skills/)，在 Claude Code 中用裸 slash 命令调用，例如 `/repo-init-gate`；命令选择器会在描述列显示 `(best-copilot)`
...
- **Claude Code**：运行 `claude --plugin-dir /absolute/path/to/best-copilot/claude-plugin`；插件默认 agent 配置是 `best-copilot:senior-project-expert`，但 CLI 通常显示不带该前缀的 agent 和 skill 名。用 `/agents` 检查插件 agent，用 `/repo-init-gate` 这类裸 slash 命令直接调用技能。

如果 Claude Code 把 Senior Project Expert 请求解析成 `Skill(senior-project-expert)` 而不是 subagent，同名兼容 skill 会先执行相同的 repo-init 预检，再允许分析、规划、派发或实现。完整 Senior Project Expert 路径仍优先使用 `/agents` 或 agent teams。
...
然后在 Claude Code 中由 **Senior Project Expert** / `senior-project-expert` 开始实质工作。该角色应将有用的仓库事实标准化到目标仓库本地文件，创建缺失的本地脚手架，并在继续实施前验证这些文件存在。
