...
- 会话主 agent 通过 `--agent senior-project-expert` 或 Claude Code 的项目/用户 `agent` 设置选择；使用 `/agents` 中显示的不带 `best-copilot:` 前缀的名字
...
然后在 Claude Code 中通过 `--agent senior-project-expert`，或项目/用户 `agent` 设置为 `senior-project-expert`，开始实质工作。该角色应将有用的仓库事实标准化到目标仓库本地文件，创建缺失的本地脚手架，并在继续实施前验证这些文件存在。
