...
Claude 适配器使用 `model: inherit`，实际模型由当前 Claude Code 会话的 `/model`、`--model` 或用户配置决定。它不会复刻 Copilot 的跨模型厂商路由。
...
| Claude Agent Teams teammate | agent 定义的 `skills:` 不会自动应用，team lead 必须显式命名技能 |
...
| 模型指定 | 具体名如 `GPT-5.4 (copilot)` | `model: inherit`（用户配置决定） |
...
| 跨模型路由 | 支持（GPT / Gemini / Claude 混合） | 仅 Claude 模型 |
...
在目标仓库上做任何实质性工作之前，系统先执行 `repo-init-gate`——只读取目标仓库根目录的 `best-copilot.md`，检查 frontmatter 中的 `version` 是否为当前契约版本 `"0.5.0"`。
...
4. execution_contract — 约束、验收检查、验证预算、上下文预算、停止条件、禁止方法
...
每个就绪任务:
  ...
  3. 要求实现证据：改了哪些文件、跑了什么测试/检查、关键输出、风险
