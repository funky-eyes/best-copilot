...
Codex 使用 `.codex-plugin/plugin.json`、`.agents/plugins/marketplace.json`、`.agents/skills -> ../skills` 和 `.codex/agents/*.toml`。Copilot CLI 通过 `plugin.json` 使用根目录 `agents/` 与 `skills/`。Claude Code 通过 `claude-plugin/` 包加载：`claude-plugin/.claude-plugin/plugin.json`、`claude-plugin/skills -> ../skills` 和 `claude-plugin/agents -> ../claude-agents`。仓库级规则保存在 `.github/instructions/**`。

## 为什么存在

大型的 AI 编码任务在从模糊的需求直接跳到补丁时常常失败。`best-copilot` 补上缺失的交付纪律：

- **一个资深入口**：由 Senior Project Expert 负责意图、范围、派发、fan-in、收口和可复用的反思/memory/策略信号。
...
- 通过插件 manifest 暴露的共享技能：[skills/](skills/)，会显示在已安装插件详情页
- 通过 [.agents/skills](.agents/skills) 直接暴露的 repo-scoped 共享技能；在当前 checkout 中工作时，这是让这些技能出现在普通 `$` skill selector 中的路径
- 通过 [.codex/agents](.codex/agents) 暴露 Codex custom agents；在当前 checkout 中工作、把该 adapter 目录复制到目标仓库，或用 Codex compatibility 跑完 `repo-init-scan` 后可用

安装或修改插件后，启动新的 Codex thread/session。需要从已安装插件使用本工作流时，显式调用 `@best-copilot`。如果希望单个 workflow skill 出现在普通 `$` skill selector 中，请通过 repo 或用户级 skill 位置暴露它们，例如 `.agents/skills` 或 `~/.agents/skills`。

Codex subagent 与插件是分开的配置面。Copilot 的 `agents/*.agent.md` 和 Claude 的 `claude-agents/*.md` 适配器不会在插件安装后自动变成 Codex custom agents，所以本仓库也在 [.codex/agents](.codex/agents) 下提供 Codex 原生 TOML adapters。对目标仓库来说，init helper 在请求 Codex compatibility 时会创建 `AGENTS.md` 和同一组 8 个 `.codex/agents/*.toml` adapter。
...
需求编排统一从 **Senior Project Expert** 这个 PM 协调者 agent 开始。它负责意图、范围、规划、派发、review fan-in、反思、memory 同步、策略增量路由和收口。
...
- **Codex**：调用 `@best-copilot`，然后要求执行该 workflow。在当前 checkout 中，`.agents/skills` 也会把兼容入口 `$senior-project-expert` 暴露给普通 `$` skill selector。需要并行工作时，必须显式要求 Codex spawn subagents；仅安装插件不会自动 spawn subagents。
...
8. PM 以 `OBSERVER_FAN_IN` 收集所有子 agent 的返回结果，再执行 `EVALUATOR_ARBITRATION`
9. 记录 `REFLECTOR_SIGNAL`，执行 `MEMORY_STATE_SYNC`，并且只对已接受、已验证的经验应用 `POLICY_DELTA_OR_NONE`
10. 调用 `/best-copilot:verification-before-completion` 后向用户交付
...
Claude 适配器在 `claude-agents/*.md` 中使用 Claude 模型别名：GPT-5.4 对应 `opus`，Gemini 对应 `haiku`，Claude Sonnet 对应 `sonnet`。在原生 Claude Code 中，这些别名会在 Claude 模型族内保留 Copilot 侧的角色档位。如果 `cc-switch`、`new-api` 或其他 Anthropic-compatible proxy 把这些别名路由到 DeepSeek、Qwen 或其他非 Claude 后端，先确认该路由会话里插件真的已启用。`/plugin list` 应显示 `best-copilot@best-copilot`，`/agents` 应显示 `best-copilot:senior-project-expert` 等 scoped agent；如果 proxy allowlist 需要显式配置，应包含 `"enabledPlugins": {"best-copilot@best-copilot": true}`。之后仍按降级模型处理，直到 PM 在读取源码或实现前输出 `PROVIDER_COMPAT -> INIT_GATE -> CLASSIFY -> PLANNER_FREEZE_PACKET -> LANE_SELECTION -> EXECUTOR_LANES`，列出所需 specialist lanes，并进入 `repo-init-gate` / `repo-init-scan` 流程。
...
| 场景 | 行为 |
|------|------|
| Codex 插件会话 | `.codex-plugin/plugin.json` 打包 skills 并显示在插件详情页；调用 `@best-copilot`，并行工作需显式要求 subagents |
| Codex repo checkout | `.agents/skills -> ../skills` 暴露共享 skills；`.codex/agents/*.toml` 暴露 Codex custom agents |
...
|------|-------|-------------|-------------|
| 入口 | 已安装插件用 `@best-copilot`；repo/custom-agent 模式用 `.codex/agents` 中的 `senior-project-expert` | `agents/pm-coordinator.agent.md` | `claude-agents/senior-project-expert.md`（通过 `--agent` 或 `.claude/settings.json`） |
...
| 技能发现 | `.codex-plugin` 提供插件详情；普通 `$` selector 来自 `.agents/skills` 或其他 skill 位置 | 从根目录 `skills/` 直接读取 | `claude-plugin/skills -> ../skills` 符号链接 |
| Agent 发现 | `.codex/agents/*.toml` custom agents | 从根目录 `agents/` 直接读取 | `claude-plugin/agents -> ../claude-agents` 符号链接 |
...
`PLANNER_FREEZE_PACKET` 和执行阶段会强制保留以下约束：
...
### 阶段六：Observer Fan-In 与 Evaluator 仲裁

PM 先把所有专员 handback 记录为可观察证据，再按优先级裁决结果。仲裁优先级：
...
### 阶段七：反思、Memory 同步、策略增量、验证与收口

收口前，PM 必须完成自进化尾部闭环：

- `REFLECTOR_SIGNAL`：提取已验证的可复用经验，或记录 `evolution_signal: none`
- `MEMORY_STATE_SYNC`：当进度、验证、收口或演进状态变化时，同步 task/spec/memory 状态
- `POLICY_DELTA_OR_NONE`：只对已接受、已验证、可回滚的经验更新路由、prompt、模板、工具优先级或 workflow 规则

随后系统执行 `verification-before-completion` 最终检查：
...
│   ├── decisions.md               ← 决策记录
│   └── workflow-rules.md          ← memory/spec/evolution 协调规则
│
...
- EvolutionEvent 记录需要 signal、target、mutation、validation、rollback、seven_module_trace、ten_pass_review、status 全部字段
...
初始化是**自动的**——不需要手动运行 `/init` 或 `copilot init`。当 PM agent 启动时，[核心工作流 Stage 1](#阶段一init-门禁强制预检) 会先运行 `repo-init-gate`；如果 `best-copilot.md` sentinel 已是当前版本，就直接跳过 `repo-init-scan`。只有 sentinel 缺失、无效、不可读或版本过期时，才进入 scan 路径采集仓库事实并创建脚手架。
...
原生 Claude Code 使用 Claude 模型别名。`claude-agents/*.md` 中的 Claude 适配器保留角色分工，并把 Copilot 档位映射为 Claude 别名：GPT-5.4 -> `opus`，Gemini 3.1 Pro (Preview) -> `haiku`，Claude Sonnet 4.6 -> `sonnet`。`cc-switch` 或 `new-api` 等 proxy route 可能把这些别名映射到非 Claude 模型；这只是 API 兼容，不代表 workflow 兼容。对 DeepSeek、Qwen 或未知后端，先确认插件已启用：`/plugin list` 应包含 `best-copilot@best-copilot`，`/agents` 应显示 scoped plugin agents；如果 `cc-switch` / `new-api` 使用 allowlist，应包含 `"enabledPlugins": {"best-copilot@best-copilot": true}`。之后在真实工作前运行非破坏性的 workflow smoke check。期望行为是：PM 输出 `PROVIDER_COMPAT -> INIT_GATE -> CLASSIFY -> PLANNER_FREEZE_PACKET -> LANE_SELECTION -> EXECUTOR_LANES`，然后为请求 dispatch 所需 lanes。收口前它还应继续经过 `OBSERVER_FAN_IN -> EVALUATOR_ARBITRATION -> REFLECTOR_SIGNAL -> MEMORY_STATE_SYNC -> POLICY_DELTA_OR_NONE -> NEXT_GATE`。如果插件已启用但它仍开始编码、跳过 init、跳过 lanes，或没有尾部门禁就收口，请改用能通过 smoke check 的模型/provider。
...
```
Planner → 冻结改进目标、目标文件、验证与回滚
Executor → 执行最小已批准变更，或返回提案
Observer → 捕获具体信号和可回放证据
Evaluator → 判断严重性、重复性、置信度和影响面
Reflector → 提取根因、经验、未来动作和反模式
Memory → 在 canonical owner 中记录 accepted/rejected/deferred 事件
Policy → 只在验证后更新有界 workflow 规则
```
...
- rollback:  ← 怎么回退（恢复方案）
- seven_module_trace: ← Planner/Executor/Observer/Evaluator/Reflector/Memory/Policy 证据
- ten_pass_review: ← 来源优先级、模块分离、范围、能力不降级、证据、memory 卫生、可回滚、运行时兼容、验证适配、未来复用
