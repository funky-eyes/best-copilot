...
`best-copilot` is a multi-runtime agent-team workflow plugin for Claude Code, Codex, and GitHub Copilot CLI. It is not a compiled application — all artifacts are Markdown (agents, skills, instructions) and JSON (manifests). There is no build step, no test suite, and no linter beyond JSON/YAML validation.

## Validation Commands

| Check | Command |
|---|---|
| Validate all JSON manifests | `ruby -rjson -e 'JSON.parse(File.read("plugin.json")); JSON.parse(File.read(".codex-plugin/plugin.json")); JSON.parse(File.read("claude-plugin/.claude-plugin/plugin.json")); JSON.parse(File.read(".claude-plugin/marketplace.json")); JSON.parse(File.read("settings.json")); JSON.parse(File.read("marketplace.json")); JSON.parse(File.read(".github/plugin/marketplace.json")); JSON.parse(File.read(".agents/plugins/marketplace.json")); puts "json ok"'` |
...
The project serves three runtimes from shared sources using an adapter pattern:
...
- **Codex layer** — `.codex-plugin/plugin.json`, `.agents/plugins/marketplace.json`, and `plugins/best-copilot/`: Codex plugin/marketplace metadata. The installable Codex subpackage has its own `.codex-plugin/plugin.json` and `skills -> ../../skills`; `.agents/skills -> ../skills` exposes the same skills for direct repo-scoped Codex use.
...
- Codex loads bundled skills through the plugin or `.agents/skills`; invoke `@best-copilot` or a bundled `$skill` explicitly when a workflow must be used. Codex subagent workflows require explicit spawn/delegation instructions and are not automatic on plugin install.
...
- After editing agents/skills/instructions, reinstall the plugin or run `/reload-plugins` in Claude Code before testing. Copilot CLI reads from its installed plugin cache; Codex local plugin tests should restart the thread/session after marketplace or plugin changes.
- When `plugin.json` version changes, update the init contract version in `best-copilot.md`, all `repo-init-*` skills, the `best-copilot.md` sample in `target-instructions-bootstrap/SKILL.md`, `.codex-plugin/plugin.json`, and all marketplace metadata.
...
- `best-copilot.md` — init sentinel (frontmatter `version: "0.8.0"`)
