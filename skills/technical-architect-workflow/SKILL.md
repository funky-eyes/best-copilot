...
5. Include a design-time assignment matrix for every proposed implementation slice: `sub_task_id`, requirement/design refs, difficulty (`high | medium | low`), owner lane, reviewer lanes, files/surfaces, write set, dependencies, parallel group, parallel readiness, acceptance checks, TDD or minimal check, verification command, ready artifacts, and stop conditions. Assign high-difficulty slices to Technical Architect, split/balance medium-difficulty slices between Technical Architect and Developer only when write sets are disjoint (no shared files, generated-template sources, or dispatch hot files), and assign low-difficulty bounded slices to Developer. Split further when a slice mixes difficulty bands, has multiple unrelated write sets, mixed owner lanes, unclear reviewer lanes, or cannot be understood by a fresh-context specialist in 2-5 minutes.
...
8. Split only when write sets can be non-overlapping, no slice edits the same file/generated-template source/dispatch hot file, and dependencies are explicit; otherwise mark the dependency and keep the task sequential under one owner instead of forcing unsafe parallelism.
