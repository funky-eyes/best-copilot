...
- If `.github/instructions/must.instructions.md` exists but lacks the Claude Code `AskUserQuestion` shape or trigger lock, append the compatible Claude Code native ask rules from this skill.
...
- If `CLAUDE.md` exists but lacks `AskUserQuestion` or `typescript-lsp@claude-plugins-official`, append only the compatible Claude runtime bullets from this skill instead of replacing existing Claude-specific rules.
...
- In Claude Code, the concrete native ask mechanism is `AskUserQuestion`: use a `questions` array, one question by default, `header` of 12 characters or fewer, 2-4 options with short descriptions, and `multiSelect: false` unless multi-select is needed. A prose next-step question is invalid when `AskUserQuestion` is available.
- Preserve Claude Code's built-in custom/Other text path. If the active integration exposes only fixed choices, include `Custom answer` and then ask a native follow-up before deciding.
...
- In Claude Code, treat `AskUserQuestion` availability as the same PM trigger signal. If a PM/coordinator message would ask the user to choose a route, approve execution, continue, or close out, call `AskUserQuestion` instead of ending with prose.
...
- If a native ask UI exists, use it for blocking route, approval, or continuation choices. In Claude Code, use `AskUserQuestion`; in VS Code, prefer the concrete `vscode_askQuestions` tool when present; in Copilot CLI, prefer `Asking user`.
...
- Code intelligence MCP is optional. Choose by current Claude tool inventory only: `mcp__gitnexus__*` first, else `mcp__codegraph__*`, else built-in Read/Grep/Glob plus shell `rg`. For TypeScript/JavaScript work, if Claude exposes LSP tools or diagnostics from `typescript-lsp@claude-plugins-official`, record `typescript_lsp_status: available` and use it for go-to-definition, references, and diagnostics before grep fallback; otherwise record `typescript_lsp_status: unavailable|not_applicable`. A local binary or plugin inventory entry is not enough. Record `code_intelligence_status: gitnexus|codegraph|unavailable`; do not call absent tools or block solely because code intelligence is unavailable.
...
- Code intelligence is optional and ordered: use `mcp__gitnexus__*` first when present, else `mcp__codegraph__*`, else built-in Read/Grep/Glob plus shell `rg`. For TypeScript/JavaScript, use exposed `typescript-lsp@claude-plugins-official` LSP tools/diagnostics before grep fallback when available.
```
...
- For TypeScript/JavaScript work, use exposed LSP tools or diagnostics from `typescript-lsp@claude-plugins-official` for go-to-definition, references, and diagnostics before grep fallback when that plugin and language server capability are available.
- When Claude Code exposes `AskUserQuestion`, PM/coordinator route choices, execution approvals, specialist `NEEDS_USER_INPUT` handbacks, continuation choices, and closeout choices must use that native popup with selectable options and a custom/Other answer path. Do not end with prose-only next-step questions.
...
- Confirm `.github/instructions/must.instructions.md` contains `## Per-Request Hard Gates`, `### No Silent Closeout`, all native-closeout bullets from this skill, the instruction-only closeout-backstop rule, Claude Code `AskUserQuestion` shape, the VS Code `vscode_askQuestions` exact-tool priority, and the specialist `NEEDS_USER_INPUT`/`BLOCKED missing_top_level_question` rule.
- Confirm `.github/instructions/must.instructions.md` contains `### PM Native Ask Trigger Gate`, including the rule that brainstorming is not the only native ask trigger, the rule that PM/coordinator frontmatter/native ask tools are an availability signal, the Claude Code `AskUserQuestion` trigger lock, and the custom free-form native ask path.
