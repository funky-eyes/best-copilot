...
- In Claude Code, `Skill(...) Successfully loaded` means the instructions were loaded only; it is not evidence that init, verification, or any file write happened. `repo-init-scan` is complete only after target files are verified on disk and its report has `required_artifacts_verified: yes`, `sentinel_written: yes`, and `next_task_ready: yes`.
- Codegraph MCP is optional. Treat it as available only when Claude exposes `mcp__codegraph__*` tools in the current session; a local binary or plugin inventory entry is not enough by itself. If codegraph is unavailable or failed to start, do not call it and do not block; use built-in Read/Grep/Glob plus shell `rg` fallback and record `codegraph_status: unavailable`.
...
- If any best-copilot specialist is invoked directly for target-repository work without a PM packet containing current `INIT_GATE` / `INIT_SCAN` evidence, run `repo-init-gate` and emit `## Repo Init Gate` before broad search, generic Explore, planning, review, or implementation; run `repo-init-scan` only if the gate fails and continue only after `## Init Summary` reports ready.
- PM/coordinator must not call codegraph or read/search business source before init completes. For standard or full work after init, PM freezes scope from the user request plus init facts and dispatches named specialists; PM-owned broad business-code exploration is not a substitute for role lanes.
...
- In Claude Code, a `Skill(...) Successfully loaded` line is not proof that init ran. Continue past first-use init only after `repo-init-scan` verifies target files on disk and reports `required_artifacts_verified: yes`, `sentinel_written: yes`, and `next_task_ready: yes`.
- PM/coordinator must not call codegraph or read/search business source before init completes. For standard or full work after init, PM dispatches named specialists instead of doing broad business-code exploration itself.
- Codegraph is optional. Use it only when `mcp__codegraph__*` tools are available in the current session; otherwise use built-in Read/Grep/Glob plus shell `rg` fallback.
```
