...
- In Claude Code, `Skill(...) Successfully loaded` means the instructions were loaded only; it is not evidence that init, verification, or any file write happened. `repo-init-scan` is complete only after target files are verified on disk and its report has `required_artifacts_verified: yes` plus `next_task_ready: yes`.
...
- Skill loading is not completion evidence. Continue past init only after the gate/scan report verifies target-local files on disk and records `next_task_ready: yes`.
...
- In Claude Code, a `Skill(...) Successfully loaded` line is not proof that init ran. Continue past first-use init only after `repo-init-scan` verifies target files on disk and reports `next_task_ready: yes`.
```
...
- A `Skill(...) Successfully loaded` line only confirms instruction loading. The first-use init gate is complete only after `repo-init-scan` verifies target-local files on disk and reports `next_task_ready: yes`.
...
- When Claude Code compatibility is required, confirm `CLAUDE.md` mentions the PM coordinator dispatch model (Agent tool for specialist subagents), foreground/background dispatch policy, isolated worktree closeout, namespaced plugin skill commands, scoped `/agents` / `@` names for plugin subagents, the unscoped `--agent senior-project-expert` first-use path with scoped fallback for collisions, and that `Skill(...) Successfully loaded` is not completion evidence.
