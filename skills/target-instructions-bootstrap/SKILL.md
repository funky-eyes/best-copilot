...
- Code intelligence MCP is optional. Treat it as available when Claude exposes `mcp__gitnexus__*` or `mcp__codegraph__*` tools in the current session; a local binary or plugin inventory entry is not enough. If neither is available, do not call code intelligence and do not block; use built-in Read/Grep/Glob plus shell `rg` fallback and record `code_intelligence_status: unavailable`.
...
- Code intelligence is optional. Use `mcp__gitnexus__*` or `mcp__codegraph__*` tools when available in the current session; otherwise use built-in Read/Grep/Glob plus shell `rg` fallback.
```
...
- For `repo-init-scan`, the next observable action after the skill load must be staged init work or `BLOCKED`, ending in `## Init Summary`. Search/read/code intelligence/project-structure exploration before that summary is invalid.
...
- Code intelligence MCP is optional. If `mcp__gitnexus__*` or `mcp__codegraph__*` tools are absent or failed in the current session, use built-in Read/Grep/Glob plus shell `rg`; if present, prefer code intelligence for structural discovery.
