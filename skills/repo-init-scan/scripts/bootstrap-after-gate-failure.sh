...
if [ -f "best-copilot.md" ] && cmp -s "best-copilot.md" - <<EOF
---
...
then
  ...
  echo "- verified_paths: best-copilot.md"
...
if [ "$compatibility" = "claude" ] || [ "$compatibility" = "claude-code" ]; then
  ...
  if [ -x "$claude_official_helper" ] || [ -f "$claude_official_helper" ]; then
    ...
    official_exit=$?
    printf '%s\n' "## Repo Init Official"
    printf '%s\n' "- attempted: claude_native_slash_init"
    printf '%s\n' "- native_command: /init"
...
if [ "$manual_exit" -eq 0 ] && [ -z "$missing_paths" ]; then
