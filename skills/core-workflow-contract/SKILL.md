...
- Use the active runtime's official initializer when available. In Copilot or other shell-capable runtimes, run `copilot init` automatically when the command exists. In Claude Code, `repo-init-scan` should use its bundled single-command helper after a missing sentinel; that helper attempts Claude Code's native `/init` command shown as "Initialize a new CLAUDE.md file with codebase documentation" through the official bounded helper (`claude --bare --permission-mode acceptEdits -p "/init"`) before deterministic manual fallback. The bare runtime `/init` command and Copilot `copilot init` command are distinct from best-copilot plugin skills such as `/repo-init-official` or `/best-copilot:repo-init-official`. Normalize official output into the project facts file. Command output without a verified project facts file, Claude-native `CLAUDE.md`, or Copilot-native init artifact is `official_init_no_write`, not success.
