...
- Code intelligence is an optional accelerator, not a dependency. In Claude Code, treat code intelligence as available when `mcp__gitnexus__*` or `mcp__codegraph__*` tools are present in the current tool inventory (gitnexus preferred); a local binary or plugin inventory entry is not enough. If neither is available, continue with Claude built-in Read/Grep/Glob plus shell `rg` where allowed; record `code_intelligence_status: unavailable` in the evidence instead of blocking. If available, prefer code intelligence for structural discovery, callers/callees, and impact checks before broad text search.
