...
- The Claude plugin ships the first-run runtime hard gate in `claude-plugin/hooks/`: `UserPromptSubmit` injects init-gate context when the target sentinel is missing or stale, `UserPromptExpansion` blocks non-init best-copilot slash commands before they expand, and `PreToolUse` denies business-source tools such as `Read`, `Grep`, `Glob`, `Bash`, `Agent`, and `mcp__*` until target `best-copilot.md` is current. This plugin-level hook is the only hook that can protect the pre-bootstrap first turn. `target-instructions-bootstrap` can also generate a target-local `.claude/hooks/best-copilot-init-gate.sh` fallback after bootstrap starts, but that fallback is persistent defense, not the initial enforcement layer. These hook gates are the enforcement layer; prompt-level MUST language is only a recovery hint.
- When the active Claude Code session is routed to DeepSeek, Qwen, or any unknown/non-Claude backend, PM still records `HARNESS_DEGRADED provider_instruction_following_unverified` until the model demonstrates it can follow the visible workflow after hook feedback. Add `provider_compatibility: hook_enforced|verified_by_smoke|unverified` to the PM packet.
- The smoke check is minimal and non-destructive: before reading business source or editing files, output `PROVIDER_COMPAT -> INIT_GATE -> CLASSIFY -> FREEZE_PACKET -> LANE_SELECTION` and name the required specialist lanes for the requested work mode. If the model starts implementation, skips init, or skips required lanes after hook feedback, stop with `BLOCKED provider_instruction_following_unverified` instead of continuing.
- Passing the hook gate and smoke check does not make the backend officially supported; it only permits the current session to continue with explicit residual risk. Prefer native Claude models or proxy models that have repeatedly passed Senior Project Expert orchestration and specialist dispatch checks.
...
- Use the active runtime's official initializer when available. In Copilot or other shell-capable runtimes, run `copilot init` automatically when the command exists. In Claude Code, `repo-init-official` must attempt the native `/init` through its bundled helper script (`claude --bare --permission-mode acceptEdits -p "/init"`) before bounded manual fallback. The bare runtime `/init` command is distinct from best-copilot plugin skills such as `/repo-init-official` or `/best-copilot:repo-init-official`. Normalize official output into the project facts file. Command output without a verified project facts file or Claude-native `CLAUDE.md` artifact is `official_init_no_write`, not success.
