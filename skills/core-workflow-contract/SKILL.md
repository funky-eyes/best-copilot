...
6. Require design-time task ownership before implementation: `design.md` names owner lanes for changed surfaces and `tasks.md` splits work into independently reviewable slices with `difficulty`, `owner_lane`, independent `reviewer_lanes`, `write_set`, `dependencies`, `parallel_group`, `parallel_ready`, acceptance checks, verification command, ready artifacts, and stop conditions. Ownership follows task difficulty first, not a Developer default: high-difficulty slices go to Technical Architect, medium-difficulty slices are balanced between Technical Architect and Developer only by disjoint write-set/dependency split; shared files, generated-template sources, and dispatch hot files force sequential ownership, and low-difficulty bounded slices go to Developer. Tasks should be small enough for a fresh-context specialist to understand in 2-5 minutes; if a task mixes difficulty bands, unrelated write sets, or reviewer lanes, route back to Specification Writer or Technical Architect for decomposition repair.
