...
- For `repo-init-scan`, the next observable action after the skill load must be staged init work (`repo-init-official` then `repo-init-manual-fallback`) or a `BLOCKED` init report, followed by `## Init Summary`. Source search/read, codegraph, planning, or specialist dispatch before that summary is the same hard protocol violation.
