...
6. Ensure `## Init Status` contains: `Init ready`, `Required artifacts verified`, `Bootstrap contract version: 0.7.1`, `Last full verification`, `Reentry rule`.
...
11. Verify: `project.instructions.md` exists and is not placeholder-heavy, `Init Status` has version `0.7.1`, `best-copilot.md` matches sentinel, and `verified_paths` covers the full runtime-required path set.
