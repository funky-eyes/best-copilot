...
5. `.codex-plugin/plugin.json` and `.agents/plugins/marketplace.json`: Codex plugin manifest and local/repo marketplace metadata.
...
- Edit Codex install metadata through `.codex-plugin/` and `.agents/`; edit Copilot installable agents through root `agents/`; edit Claude Code adapters through root `claude-agents/`; edit shared installable skills through root `skills/`; edit repository instructions through `.github/instructions/**`.
